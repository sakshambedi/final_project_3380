...
class Gui:
  def __init__(self):
    ...
    view_table = ttk.Button(ws, text = 'View Tables', command = self.view_table , width =75 )
    top_3_winning = ttk.Button(ws, text = 'Top 3 Teams(Winning)', command = self.top_3_winning, width =75)
    top_3_winning_away = ttk.Button(ws, text = 'Top 3 Away Teams Winning', command = self.top_3_winning_away, width =75)
    top_3_winning_special = ttk.Button(ws, text = 'Top 3 Most Wins and Number of Wins', command = self.top_3_special, width =75)
    player_query4 = ttk.Button(ws, text = 'Top 3 Players played most seasons', command = self.player_query4, width =75)
    ref_every_game = ttk.Button(ws, text = 'All referees that have overseen all games', command = self.ref_every_game, width =75)
    players_double = ttk.Button(ws, text = 'Players with double-double', command = self.players_double, width =75)
    player_triple = ttk.Button(ws, text = 'Players with most triple-double', command = self.player_triple, width =75)
    player_closeto_triple = ttk.Button(ws, text = 'Players that are close to the triple-double', command = self.player_closeto_triple, width =75)
    arena_most_games = ttk.Button(ws, text = 'Arena with most games', command = self.arena_with_most_games, width =75)
    arena_most_points = ttk.Button(ws, text = 'Arena with most points', command = self.arena_most_points, width =75)
    arena_least_points = ttk.Button(ws, text = 'Arena with least points', command = self.arena_least_points, width =75)
    team_most_points_combined = ttk.Button(ws, text = 'Team with most points combined', command = self.team_most_points_combined, width =75)
    non_nba_not_drafted = ttk.Button(ws, text = 'Non-NBA Team Player not drafted', command = self.non_NBA_not_drafted, width =75)
    average_weight_player = ttk.Button(ws, text = 'Average weight of drafted players', command = self.average_weight_players, width =75)
    ref_seen_most_points = ttk.Button(ws, text = 'Referee that saw most points', command = self.ref_seen_most_points, width =75)
    player_query26 = ttk.Button(ws, text = 'Countries with most NBA players and number of players', command = self.player_query26, width =75)
    average_points_1st = ttk.Button(ws, text = 'Average points of 1st round 1st pick of all draft', command = self.average_points_1st, width =75)
    average_1st_round_30th_pick = ttk.Button(ws, text = 'Average points of 1st round 30th pick of all draft', command = self.average_1st_round_30th_pick, width =75)
    find_dallas_player = ttk.Button(ws, text = 'Dallas player playing for 10 years ', command = self.find_dallas_player, width =75)
    count_num_player_country = ttk.Button(ws, text = 'Count number of players in each country ordered desc', command = self.count_num_player_country, width =75)
    # player_attende_7_allstar = ttk.Button(ws, text = 'Players that attended all 7-star games', command = self.player_attende_7_allstar, width =75)
    # most_all_star = ttk.Button(ws, text = 'Team with most all-star players in history', command = self.most_all_star, width =75)
    player_longest_played = ttk.Button(ws, text = 'Player with longest played years in NBA', command = self.player_longest_played, width =75)
    team_highest_mark_game = ttk.Button(ws, text = 'Team with highest mark in a game in history', command = self.team_highest_mark_game, width =75)
    top_5_oversea_players = ttk.Button(ws, text = 'Top 5 players from oversea countries and their teams', command = self.top_5_oversea_players, width =75)
    best_player_NBA = ttk.Button(ws, text = 'Best player in NBA history using given formula', command = self.best_player_NBA, width =75)
    worst_team_NBA = ttk.Button(ws, text = 'Worst team in NBA history using given formula', command = self.worst_team_NBA, width =75)
    team_wins_home_season = ttk.Button(ws, text = 'Team with most wins in home in a certain season', command = self.team_wins_home_season, width =75)
     
    view_table.pack(pady=2)
    top_3_winning.pack(pady=2)
    top_3_winning_away.pack(pady=2)
    top_3_winning_special.pack(pady=2)
    player_query4.pack(pady=2)
    ref_every_game.pack(pady=2)
    players_double.pack(pady=2)
    player_triple.pack(pady=2)
    player_closeto_triple.pack(pady=2)
    arena_most_games.pack(pady=2)   
    arena_most_points.pack(pady=2)
    arena_least_points.pack(pady=2)
    team_most_points_combined.pack(pady=2)
    non_nba_not_drafted.pack(pady=2)
    average_weight_player.pack(pady=2)
    ref_seen_most_points.pack(pady=2)
    player_query26.pack(pady=2)
    average_points_1st.pack(pady=2)
    average_1st_round_30th_pick.pack(pady=2)
    find_dallas_player.pack(pady=2)
    count_num_player_country.pack(pady=2)
    # player_attende_7_allstar.pack(pady=2)
    # most_all_star.pack(pady=2)
    player_longest_played.pack(pady=2)
    team_highest_mark_game.pack(pady=2)
    top_5_oversea_players.pack(pady=2)
    best_player_NBA.pack(pady=2)
    worst_team_NBA.pack(pady=2)
    team_wins_home_season.pack(pady=2)
